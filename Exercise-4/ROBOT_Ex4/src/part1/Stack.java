...
public class Stack<_action, _state> implements Agenda<Node<_action, _state>>
{
	...
	public Node<_action, _state> pop() {
		int lastItem = frontier.size() - 1;
		explored.add(frontier.get(lastItem).getData().getState());
		return frontier.remove(lastItem);
	}
